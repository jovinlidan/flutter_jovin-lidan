...
class _ContactCreateScreenState extends State<ContactCreateScreen> {
  ...
  void handleSubmit(BuildContext context) async {
    if (_formKey.currentState!.validate()) {
      ...
      final contact = await Provider.of<ContactCreateViewModel>(context, listen: false)
          .postContact(contact: Contact(id: 4, name: fullName!, phone: phoneNumber!));
      if (contact?.status == ApiStatus.success) {
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Success")));
        Navigator.pop(context);
      } else {
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text("${contact?.message}")));
      }
    }
  }
          ...
          child: Column(children: [
            ...
            Consumer<ContactCreateViewModel>(
              builder: (_, value, __) => ElevatedButton(
                onPressed:
                    value.contact?.status == ApiStatus.loading ? null : () => handleSubmit(context),
